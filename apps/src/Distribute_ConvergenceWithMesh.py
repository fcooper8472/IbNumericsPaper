...
def run_simulations():
    ...
    # 2^5 (32) up to 2^12 (4096) at half-integer powers, rounded to integer value
    param_values = (2**(np.linspace(5, 12, num=num_sims))).astype(int)
...
def plot_results():
    ...
    mesh_spacing = -1.0 * np.log2(esf_data[:, 1])
    ...
    plt.plot(mesh_spacing, esf_at_end, marker='o',
             linestyle='-',
             linewidth=0.75,
             color=mycol_or,
